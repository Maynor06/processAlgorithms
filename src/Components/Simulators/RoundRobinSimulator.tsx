...
interface Props {
  ...
  isPaused: boolean;
  resetFlag: boolean;
  quantum: number; // <- Quantum recibido desde el componente padre
}
    ...
    {/* === Visualización Gantt === */}
      <div className="flex-1 bg-white shadow rounded-xl p-4 overflow-y-auto">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-bold">Visualización Round Robin</h2>
          <div className="text-xs text-gray-500">
            TIME_UNIT: {TIME_UNIT} ms · Quantum: {quantum} · Estado:{" "}
            {isComplete
              ? "Finalizado"
              : isPaused
              ? "Pausado"
