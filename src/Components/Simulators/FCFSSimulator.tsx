...
export default function FCFSSimulator({ isRunning, isPaused, resetFlag }: Props) {
  ...
  // 🔹 Adaptar procesos del contexto al formato interno
  ...
  // Reiniciar motor cuando cambien los procesos o se pida reset
  useEffect(() => {
    if (data.length === 0) {
  ...
  // Mapa: tiempo → snapshot de cola
  const queuePosByTime = useMemo(() => {
    const map = new Map<number, Map<number, number>>();
    steps.forEach((s) => {
      const inner = new Map<number, number>();
      s.queueBefore.forEach((pid, idx) => {
        if (idx > 0) inner.set(pid, idx); // idx=0 es CPU, >0 cola
      });
                ...
                return (
                  ...
                  >
                    ...
                    {Array.from({ length: MAX_COLS }).map((_, t) => {
                      ...
                      // ✅ solo mostrar en cola si ya llegó
                      const procArrival =
