...
/** Color estable por PID (HSL) para el bloque en CPU del Gantt */
function colorForPid(pid: number) {
  const hue = (pid * 67) % 360;
  return `hsl(${hue} 70% 55%)`;
}

/** Centrar texto horizontalmente en una celda PDF de ancho w */
...
export default function SRTFSimulator({ isRunning, isPaused, resetFlag }: Props) {
  const { procesos } = useProcesoContext();

  // En SRTF usamos el PID propio del proceso
  ...
  // Reinicio cuando cambian procesos o se pide reset
  useEffect(() => {
    setSteps([]);
    setResults([]);
    setIsComplete(false);
  ...
  // Arranque “instantáneo”: calculo todo el timeline, muestro overlay TIME_UNIT y pinto resultados
  useEffect(() => {
    ...
    if (isRunning && !isPaused && !isComplete) {
      ...
      eng.onStep((s) => bufSteps.push(s));
      eng.onFinish((r) => bufResults.push(r));
      eng.onComplete(() => { /* se marca después del volcado */ });

      // Ejecutar todo sin intervalos
      let done = false;
      while (!done) {
        done = eng.tick();
      }

      // Overlay del mismo largo que TIME_UNIT
      startOverlay(TIME_UNIT);
      const t = window.setTimeout(() => {
        setSteps(bufSteps);
        // ordenar resultados por nombre (como en otros simuladores)
  ...
  // Mapa: tiempo → (pid → posición en cola, excluyendo el que corre en CPU)
  ...
  function exportPdfPure() {
    ...
    // Encabezados: sin relleno, solo borde negro y texto negro
    pdf.setDrawColor(0);
    pdf.setTextColor(0);
    let x = tableX;
    headers.forEach((h, i) => {
  ...
  return (
    <div className="flex flex-col h-full gap-4 relative">
                      ...
                      return (
                        <td
                          ...
                          className={
                            ...
                            (isRunningHere ? "text-white font-bold" : "text-slate-400")
                          }
      ...
      <div className="flex-1 bg-slate-50 border border-slate-200 rounded-xl shadow-sm p-4 overflow-y-auto">
        ...
        {/* Botón Exportar PDF – en el contenedor gris */}
